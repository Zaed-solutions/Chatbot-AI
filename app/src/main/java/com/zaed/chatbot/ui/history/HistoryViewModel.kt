...
class HistoryViewModel(
    ...
    private fun fetchChatHistories() {
        viewModelScope.launch(Dispatchers.IO) {
            chatRepo.getChatHistories().onSuccess { histories ->
    ...
    private fun deleteHistory(chatId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            chatRepo.deleteChatHistory(chatId).collectLatest { result ->
                result.onSuccess {
                    fetchChatHistories()
                }.onFailure { e ->
                    Log.e(TAG, "deleteHistory error: ${e.localizedMessage}")
                }
            }
        }
    }
    ...
    private fun renameHistory(chatId: String, newTitle: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val newHistory =
                uiState.value.histories.find { it.chatId == chatId }?.copy(title = newTitle)
            newHistory?.let {
                chatRepo.updateChatHistory(it).collectLatest { result ->
                    result.onSuccess {
                        fetchChatHistories()
                    }.onFailure { e ->
                        Log.e(TAG, "renameHistory error: ${e.localizedMessage}")
                    }
                }
            }
        }
